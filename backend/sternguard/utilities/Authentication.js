...
/**
 *
 * @param {email} email for which to reset password link will be generated
 */
const generateResetPasswordToken = function (email) {
	const expiryTime = Math.floor(Date.now() / 1000 + 3600 * 5); // 3600 * 5 = 5 hours
 ...
 * @param {email} email for which to reset password link will be generated
 */
const generateInviteToken = function (email, role_id) {
	const expiryTime = Math.floor(Date.now() / 1000 + 3600 * (24 * 14)); // 3600 * (24 * 14) = 14 days
	const inviteCandidateSecrete =
        process.env.INVITE_CANDIDATE_TOKEN_SECRET_KEY;

	return getToken({ email: email, role_id: role_id }, expiryTime, inviteCandidateSecrete);
};
