...
module.exports = {
	...
	updateStripeSubscription: async function ({
	...
	getStripeSubscriptionProration: async (
		ip_address,
		stripe_customer_id,
		stripe_subscription_id,
		{ stripe_pricing_id, coupon },
		trial_period_days = 0
	) => {
		try {
			if (
				...
				throw new Error(
					'For existing customer both customer id and subscription id is required'
				);
			}

			let isOldCustomer =
				stripe_customer_id && stripe_subscription_id ? true : false;
			...
			var options = {
				...
				subscription_items: [{ price: stripe_pricing_id }],
				...(trial_period_days > 0
					? {
							subscription_trial_end:
								Math.floor(Date.now() / 1000) +
								trial_period_days * 24 * 3600,
					  }
					: null),
				expand: ['subscription'],
				coupon: coupon,
	...
	attachCustomerPaymentMethod: async (
		stripe_customer_id,
		{ payment_method_id, make_default_payment_method }
	) => {
		try {
			...
			const defaultPaymentMethod = make_default_payment_method
